...
public class Save
{
	...
	/**
	 * Writes input name to scores.txt file in main directory Will create if
	 * file does not exist and append if it does parameter name string to write
	 *
	...
	public void write(String name, Player user)
	{
		 ...
		 * throw if successful name and score will be written to file then
		 * closed
		 */
		try
		{
	...
	public void update(Player user)
	{
		...
		try
		{
			...
			/**
			 * Compares sorted int values to whole score entries in order to
			 * sort scores.txt based on scores
			 */
	...
	public void read()
	{
		 ...
		 * throw if successful names will be read and copied to names ArrayList
		 * then scores will copied to scores ArrayList closed
		 */
		try
		{
			FileReader fReader = new FileReader(namesFile);
	...
	/**
	 * Checks number of lines in scores.txt and clears file if greater than or
	 * equal to 6
	 */
	public void lineCheck()
	{
		 ...
		 * throw if successful scores.txt will be cleared out if more than 6
		 * lines then closed
		 */
		try
		{
